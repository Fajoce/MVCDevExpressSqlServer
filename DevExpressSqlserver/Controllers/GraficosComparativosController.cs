...
{
    [Authorize]
    public class GraficosComparativosController : Controller
    {
        private readonly SBERPDbContext _context;

        public GraficosComparativosController(SBERPDbContext context)
        {
        ...
        [HttpGet]
        public IActionResult GetGrafico(string values = null)
        {
            // Obtener el ID del usuario logueado
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId))
            ...
            int usuarioId = int.Parse(userId); // Si tu ID de usuario es int, cambia a Guid.Parse si es GUID

            // Inicializar variables de filtro
            DateTime? fechaInicio = null;
            DateTime? fechaFin = null;

            // Leer los valores del parámetro JSON 'values'
            if (!string.IsNullOrEmpty(values))
            {
                try
                {
            ...
            // Consultar movimientos filtrados por usuario y fecha
            ...
            // Agrupar por tipo de movimiento y calcular el total
            var resultado = query
